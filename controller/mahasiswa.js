...
exports.addMahasiswa = async (req, res) => {
  try {
    const { nim, nama, jurusan } = await insertMahasiswa.validateAsync(
      req.body
    );
    const insert = {
      id: uuidv4(),
...
exports.updateMahasiswa = async (req, res) => {
  try {
    const { id } = await getOne.validateAsync(req.params);
    const { nim, nama, jurusan } = await updateMahasiswa.validateAsync(
      req.body
    );
...
exports.getGroupMatakuliah = async (reg, res) => {
  try {
    const groupMatakuliah = await knex("krs")
      .leftJoin("matakuliah as mk", "mk.id", "krs.id_matakuliah")
